        ...
        li.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center;">
                <div class="cust-db-name">
                    <h5>${c.name}</h5>
                    <p>🏡 ${c.tower} ${c.floor}0${c.flat} &bull; 📱 ${c.phone}</p>
                </div>
                <div class="cust-action-pings">
...
function submitCustomerLogin() {
    ...
    // Check if phone matches any known customer
    const match = state.customersList.find(c => c.phone === phone || c.phone.replace(/\D/g, '') === phone);
    if (match) {
        state.customerProfile.name = match.name;
        state.customerProfile.phone = match.phone;
        state.customerAddress.tower = match.tower;
        state.customerAddress.floor = match.floor;
        state.customerAddress.flat = match.flat;
        showToast(`👋 Welcome back, ${match.name}!`, "success");
    } else {
        // Create new user profile
        state.customerProfile.name = "Resident";
        state.customerProfile.phone = phone;
        state.customerAddress.tower = "Alexander";
        state.customerAddress.floor = "1";
        state.customerAddress.flat = "1";
        showToast("👋 Welcome to Anshaisha Kitchen!", "success");
    }
